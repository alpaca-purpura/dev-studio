...
func NewRepoService(ctx context.Context, store ports.RepoStore) (*RepoService, error) {
	repos, err := store.LoadRepos(ctx)
	if err != nil {
		return nil, fmt.Errorf("repo service: load: %w", err)
	}
	return &RepoService{repos: repos, store: store, ctx: ctx}, nil
}

// Register valida que la ruta sea un repo git local y lo registra. Misma ruta ya
// registrada → devuelve el existente (idempotente, no duplica).
